...
pieces_train = []
actions_train = []
game_step = [100,200,300,400,500]
action_step = [50,100,200,300]
upper_bound = 30 # initial value
idx_bound = 0 # initial value

for i_game in range(num_games_train):
    if i_game in game_step:
        if i_game == 500:
            upper_bound = -1
        else:
            upper_bound = action_step[idx_bound]
            idx_bound += 1
    ...
    for t in count():
        ...
        reward, done = env.step(action.item(), False)
        reward = torch.tensor([reward])
        score += gamma*reward

        if t == upper_bound: # restrict max actions per game
            done = True
