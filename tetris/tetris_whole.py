...
def get_state_t(board, tetromino, rot, x_pos):
    ...
    state_t = [[0 if board[y][x] == 0 else 1 for x in range(cols)] for y in range(rows)]
    flat_state = [item for l in state_t for item in l]
    end_state = flat_state + tetromino + rot + [x_pos]

    return end_state, height, bumpiness, tower, holes, level
...
class TetrisApp(object):
    ...
    def step(self, action_number, screen):
        ...
        reward = lines - 0.01 * height - 0.09 * bumpiness - 0.6 * max(tower,0) - 0.5 * holes - 0.1 * level
