...
class TetrisApp(object):
    ...
    def get_perf(self):
        return self.score, self.lines, self.pieces-1, self.actions
    ...
    def get_next_states(self):
        ...
        valid_xs = cols - len(curr_piece[0])
        for x in range(valid_xs + 1): # for each position (0-8)
            self.stone_x = x
    ...
    def step_grouped(self, action_number, screen):
        ...
        if not check_collision(self.board, self.stone, (action_number, self.stone_y)):
            self.stone_x = action_number
