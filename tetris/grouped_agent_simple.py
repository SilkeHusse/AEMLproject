...
from tetris.grouped_tetris_simple import *
...
for i_game in range(num_games_train+1):
    ...
    while t < T:
        ...
        print(len(states))
        if t == 0:
            action = torch.tensor([random.randrange(len(states))], dtype=torch.long) # random first action (in even place, nice!)
        else:
            action = select_action(states)
        reward, done = env.step_grouped(action.item(), show)
        if reward >= 5: # at least cleared one line
            T += 10
    ...
    if i_game % 100 == 0:
        plot_rewards()
        torch.save(policy_net.state_dict(), "J_drop_eps_{}_alt.pt".format(i_game))
