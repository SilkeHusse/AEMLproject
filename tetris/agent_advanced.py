...
plt.figure(2)
plt.clf()
plt.title('Pieces - Training') # average number of actions per game
plt.xlabel('Game')
plt.ylabel('Pieces')
plt.xticks(np.arange(len(pieces_train)), np.arange(1, len(pieces_train)+1))
plt.plot(pieces_train)
#plt.axhline(avg_actions_train, color="#FFA500")
plt.locator_params(axis='x', nbins=10)
plt.show()
...
plt.figure(3)
plt.clf()
plt.title('Scores - Training') # average number of actions per game
plt.xlabel('Game')
plt.ylabel('Score')
plt.xticks(np.arange(len(scores_train)), np.arange(1, len(scores_train)+1))
plt.plot(scores_train)
plt.axhline(avg_scores_train, color="#FFA500")
plt.locator_params(axis='x', nbins=10)
plt.show()
...
for i_game in range(num_games_test):
    ...
    for t in count():
        ...
        if action != 5:
            not_hard_drop += 1
        if not_hard_drop > 50: # force hard drop after 50 non hard drop actions
            action = torch.tensor([5], dtype=torch.long)
            not_hard_drop = 0
            forced_action += 1
        _, done = env.step(action.item(), show)
...
plt.figure(6)
plt.clf()
plt.title('Pieces - Testing') # average number of actions per game
plt.xlabel('Game')
plt.ylabel('Pieces')
plt.xticks(np.arange(len(pieces_test)), np.arange(1, len(pieces_test)+1))
plt.plot(pieces_test)
#plt.axhline(avg_actions_test, color="#FFA500")
plt.locator_params(axis='x', nbins=10)
plt.show()
...
plt.figure(7)
plt.clf()
plt.title('Scores - Testing') # average number of actions per game
plt.xlabel('Game')
plt.ylabel('Score')
plt.xticks(np.arange(len(scores_test)), np.arange(1, len(scores_test)+1))
plt.plot(scores_test)
plt.axhline(avg_scores_test, color="#FFA500")
plt.locator_params(axis='x', nbins=10)
plt.show()
